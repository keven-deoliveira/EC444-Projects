/*
  ...
  Adapted again by Keven DeOliveira, 03/18/2022 for Quest 3
*/
...
char payloadTemp[MAX_PAYLOAD_SIZE];

char payloadX[MAX_PAYLOAD_SIZE];
char payloadY[MAX_PAYLOAD_SIZE];
char payloadZ[MAX_PAYLOAD_SIZE];

char payloadP[MAX_PAYLOAD_SIZE];
char payloadR[MAX_PAYLOAD_SIZE];
...
static void udp_client_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            strcat(payloadX, ",");
            strcat(payloadX, payloadY);
            strcat(payloadX, ",");
            strcat(payloadX, payloadZ);
            strcat(payloadX, ",");
            strcat(payloadX, payloadP);
            strcat(payloadX, ",");
            strcat(payloadX, payloadR);
            strcat(payloadX, ",");
            strcat(payloadX, payloadTemp);
            ...
            // Data received
                ...
                // logic for button toggling
                if (strncmp(rx_buffer, "ON", 2) == 0) ledStatus = 1;
                if (strncmp(rx_buffer, "OFF", 3) == 0) ledStatus = 0;
...
void calcRP(float x, float y, float z){
  ...
  sprintf(payloadR, "%f", roll);
  sprintf(payloadP, "%f", pitch);
...
static void test_adxl343() {
  ...
  while (1) {
    ...
    sprintf(payloadX, "%f",xVal);
    sprintf(payloadY, "%f",yVal);
    sprintf(payloadZ, "%f",zVal);
...
static void thermistor(void) {
    ...
    while (1) {
        ...
        voltage_split = 3300 - voltage;
        

        // calculate resistance
        res = (RESISTANCE * (voltage_split/RESISTANCE)) / (voltage/RESISTANCE);

        printf("res: %.2f\n", res);

        // calc temperature (in celsius) by using BCOEF and res
        temp = 1.0 / ((1.0/298.15) - (1.0/BCOEF) * log(res/RESISTANCE));
        temp -= 273.15;
        temp += 27;

        //payloadTemp = temp;
        sprintf(payloadTemp, "%g", temp);

        printf("TEMPERATURE (*C): %.2f", temp);
        printf("\n\n");
...
void app_main() {
  ...
  xTaskCreate(thermistor, "thermistor", 4096, NULL, 6, NULL);

  // Create task to poll ADXL343
  xTaskCreate(test_adxl343,"test_adxl343", 4096, NULL, 5, NULL);

 xTaskCreate(ledHandler, "ledHandler", 4096, NULL, tskIDLE_PRIORITY, NULL);
